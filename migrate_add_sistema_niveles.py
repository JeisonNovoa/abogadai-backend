"""
Migración: Sistema de Niveles, Límites de Sesiones y Reembolsos
Fecha: 2025-12-22

Agrega:
- Campos de niveles y límites a tabla users
...
def migrate():
    ...
    try:
        ...
        print("\n📊 Paso 3: Actualizando enum 'estadocaso'...")
        ...
        try:
            ...
            for valor in nuevos_valores:
                if valor not in valores_actuales:
                    ...
                    print(f"   ⏭️  Valor '{valor}' ya existe en enum")

        except Exception as e:
            # Si no es PostgreSQL o enum no existe, omitir
            print(f"   ⏭️  Enum update no aplicable (probablemente SQLite): {str(e)[:50]}")
            db.rollback()

        # 4. CREAR TABLA SESIONES_DIARIAS
        print("\n📊 Paso 4: Creando tabla 'sesiones_diarias'...")
        ...
        try:
            ...
            try:
                ...
                db.rollback()
                print("   ⏭️  Enums no aplicables (SQLite)")
        ...
        print("\n📊 Paso 6: Migrando datos existentes...")
        ...
        print(f"   📄 Encontrados {len(casos_pagados)} casos ya pagados")
        ...
        print("\n📊 Paso 7: Recalculando niveles de usuarios...")
